    ...
    public class CCPlayerController : PlayerController
    {
        ...
        public override void OnGrounded()
        {
            ...
            if(!IsLanded)
            {
                ...
                if(AudioManager.IsInstanced)
                {
                    if(!GameManager.IsInstanced) { AudioManager.Instance.InteractWithSFX("Land", SoundEffectBehaviour.Play); return; }
        ...
        public override void OnNotGrounded()
        {
            ...
            if(GravVel.y > 0f && !IsJumped)
            {
                ...
                if(AudioManager.IsInstanced)
                {
                    if(!GameManager.IsInstanced) { AudioManager.Instance.InteractWithSFX("Jump", SoundEffectBehaviour.Play); return; }
